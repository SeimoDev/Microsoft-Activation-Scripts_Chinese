...
::============================================================================
::
::   This script is a part of '微软激活脚本【由Seimo汉化】' (MAS) project.
::
...
if %errorlevel% NEQ 0 (
echo:
echo 服务未在运行，程序可能已经崩溃...
echo:
echo:
echo 帮助（英文） - %mas%troubleshoot.html
echo:
echo:
ping 127.0.0.1 -n 10
)
...
>nul findstr /v "$" "%~nx0" && (
echo:
echo 错误: 脚本行尾的换行符可能存在问题，或者脚本末尾可能缺少一个空行。
echo:
...
%nceline%
echo 不支持的系统版本 [%winbuild%].
echo 本项目仅支持Windows7/8/8.1/10/11以及同等底层的Windows Server。
goto MASend
)

for %%# in (powershell.exe) do @if "%%~$PATH:#"=="" (
%nceline%
echo 系统内未找到powershell.exe。
echo 程序即将终止...
goto MASend
)
...
echo:             [3] KMS38       ^|  Windows           ^|   到 2038 年
echo:             [4] Online KMS  ^|  Windows / Office  ^|   续期 180 天
...
echo:       ______________________________________________________________
echo:
call :_color2 %_White% "                  " %_Green% "通过键盘输入菜单选项 [1,2,3,4,5,6,7,8,0] :"
echo:                           由 seimo.cn 汉化
...
echo:       ______________________________________________________________
echo:
echo:             [1] 修改Windows系统版本
echo:
echo:             [2] 提取 $OEM$ 文件夹
echo:
echo:             [3] 查看当前系统激活状态 [vbs]
echo:
...
echo:       ______________________________________________________________
echo:
call :_color2 %_White% "           " %_Green% "通过键盘输入菜单选项 [1,2,3,4,0] :"
choice /C:12340 /N
set _erl=%errorlevel%
...
echo:           ________________________________________________________
echo:  
call :_color2 %_White% "           " %_Green% "通过键盘输入菜单选项："
...
if defined _error (
%eline%
echo 提取 $OEM$ 文件夹到桌面失败。
) else (
echo:
call :_color %Blue% "%_oem%"
call :_color %Green% "成功提取 $OEM$ 文件夹到桌面。"
)
echo "%_oem%" | find /i "KMS38" 1>nul && (
echo:
echo KMS38激活服务器核心/辅助版本（无图形用户界面） ,
...
if %~z0 GEQ 200000 (
set "_exitmsg=返回"
set "_fixmsg=返回主菜单，选择 "故障排除 "并运行 "修复 Licensing "选项。"
) else (
set "_exitmsg=退出"
set "_fixmsg=在 MAS 文件夹中，运行 "故障排除 "脚本并选择 "修复 Licensing "选项。"
)
...
call :dk_color %Red% "启动 [sppsvc] 服务失败，剩余过程可能需要很长时间..."
echo:
)
...
echo:
call :dk_color2 %_White% "     " %Green% "检查: %winos% 已被永久激活"
...
call :dk_color %Red% "Checking Alternate Edition For HWID     [%altedition% 未找到激活 ID]"
)
...
call :dk_color2 %Blue% "查看此页以获得帮助" %_Yellow% " %mas%troubleshoot"
goto :dl_final
)
...
call :dk_color %Red% "生成 GenuineTicket.xml            [Failed, aborting the process]"
echo [%encoded%]
if exist "%tdir%\Genuine*" del /f /q "%tdir%\Genuine*" %nul%
goto :dl_final
) else (
...
set error=1
if exist "%tdir%\*.xml" del /f /q "%tdir%\*.xml" %nul%
call :dk_color %Red% "Installing GenuineTicket.xml            [ClipSVC 服务重启失败，等待中...]"
)
)
...
call :dk_color2 %Blue% "查看此页以获得帮助" %_Yellow% " %mas%licensing-servers-issue"
)
)
...
set "_exitmsg=退出"
set "_fixmsg=在 MAS 文件夹中，运行 "故障排除 "脚本并选择 "修复许可 "选项。"
)
...
%eline%
echo 检测到不支持的操作系统版本 [%winbuild%]。
echo Ohook Activation 支持 Windows 8 及更高版本及其同底层的Windows Server。
goto dk_done
)
...
echo         ____________________________________________________________
echo: 
call :dk_color2 %_White% "              " %_Green% "通过键盘输入菜单选项 [1,2,3,0]"
choice /C:1230 /N
set _el=!errorlevel!
if !_el!==4  exit /b
...
echo:
call :dk_color %Red% "检查不支持的 Office 安装程序     [ %o14msi%%o14c2r%%o16uwp%]"
)
...
echo:
echo 您只安装了 Office dashboard 应用程序，您需要安装完整的 Office 版本。
)
echo:
...
call :dk_color %Red% "Removing Office vNext Block             [失败]"
) || (
echo 移除 Office vNext 块             [成功]
)
)
...
echo:
echo 删除 - HKCU\Software\Microsoft\Office\16.0\Common\Licensing\Resiliency
...
echo 将系统的 sppc.dll 连接到         ["%_hookPath%\sppcs.dll"] [成功]
echo 提取自定义 %_hook% 到         ["%_hookPath%\sppc.dll"] [成功]
) else (
set error=1
call :dk_color %Red% "将系统的 sppc.dll 连接到             [失败]"
call :dk_color %Red% "提取自定义 %_hook% 到            [失败]"
...
%eline%
echo 检测到不支持的操作系统版本 [%winbuild%].
echo KMS38 激活支持 Windows 10/11/Server，14393 及更高版本。
goto dk_done
)
...
echo         ____________________________________________________________
echo: 
call :dk_color2 %_White% "              " %_Green% "通过键盘输入菜单选项 [1,2,0]"
choice /C:120 /N
set _el=!errorlevel!
if !_el!==3  exit /b
...
echo Windows Server评估版本无法激活。将其转换为完整的Windows Server操作系统。
echo:
echo 在 MAS 中，进入 杂项 并使用 "修改Windows系统版本 "选项。
) else (
echo 评估版无法激活。
echo 您需要安装 %winos% 的完整版本
echo:
...
%eline%
echo 服务器 Cor/Acor [无图形用户界面] 版本中不存在 clipup.exe。
echo 它是 KMS38 激活所必需的。
echo 请查看以下页面了解如何激活。
echo %mas%kms38.html
goto dk_done
)
...
call :dk_color %Blue% "要激活 IotEnterpriseS KMS38，Windows 必须升级到 19044.2788 版或更高版本。"
)
)

if not defined key if defined notfoundaltactID (
call :dk_color %Red% "检查 KMS38 的备用版本   [%altedition% 未找到激活 ID]"
)
...
echo:
if defined changekey (
call :dk_color %Blue% "[%altedition%] 版本产品密钥将被用于激活 KMS38。"
echo:
)
...
if not defined app (
call :dk_color %Red% "检查已安装的 GVLK 激活 ID   [Not Found] 程序中止..."
call :dk_color2 %Blue% "查看此页以获得帮助" %_Yellow% " %mas%troubleshoot"
goto :dk_done
)
...
echo:
if exist "!_clipup!" (
echo 将 clipup.exe 文件复制到              [%systemroot%\System32\] [成功]
) else (
call :dk_color %Red% "将 clipup.exe 文件复制到              [%systemroot%\System32\] [失败] 程序中止..."
goto :k_final
)
)
...
if not exist %SystemRoot%\system32\ClipUp.exe (
call :dk_color %Red% "检查 ClipUp.exe 文件                [Not Found, 中止进程]"
call :dk_color2 %Blue% "查看此页以获得帮助" %_Yellow% " %mas%troubleshoot"
goto :k_final
)
...
set error=1
if exist "%tdir%\*.xml" del /f /q "%tdir%\*.xml" %nul%
call :dk_color %Red% "安装 GenuineTicket.xml            [ClipSVC 服务重启失败，等待中...]"
)
)
...
set error=1
set rebuildinfo=1
call :dk_color %Red% "检查 ClipSVC tokens.dat             [Not Found]"
)

%_xmlexist% (
set error=1
set rebuildinfo=1
call :dk_color %Red% "安装 GenuineTicket.xml            [Failed With clipup -v -o]"
)
...
if %errorlevel%==0 (
echo 激活...应用 SKU-ID Rearm                   [成功]
) else (
call :dk_color %Red% "激活...应用 SKU-ID Rearm                   [失败]"
)
call :dk_refresh
...
%nul% reg delete "HKU\S-1-5-20\%specific_kms%" /f
%nul% reg query "HKLM\%specific_kms%" && (
call :dk_color %Red% "删除已添加的特定 KMS 主机    [失败]"
) || (
echo 删除已添加的特定 KMS 主机    [成功]
)
)
...
%nul% reg delete "HKLM\%specific_kms%" /f
%nul% reg query "HKLM\%specific_kms%" && (
echo 从 KMS 保护 KMS38                  [成功] [锁定注册表密钥]
) || (
call :dk_color %Red% "从 KMS 保护 KMS38                  [Failed To Lock A Registry Key]"
)
)
...
echo.              [3] 激活 - 全部
echo.
if defined _tskinstalled call :_color2 %_White% "              [4] 安装自动续订功能      " %_Green% "[Installed]"
if defined _oldtsk       call :_color2 %_White% "              [4] 安装自动续订功能      " %_Red% "[Old Installed]"
if not defined _tskinstalled if not defined _oldtsk echo.              [4] 安装自动续订功能
echo.              [5] 卸载
...
call :_color2 %_White% "              [6] 开启调试模式         " %_Red% "[Yes]"
)
...
echo.       ______________________________________________________________
echo.
call :_color2 %_White% "           " %_Green% "通过键盘输入菜单选项 [1,2,3,4,5,6,7,0]"
choice /C:12345670 /N
set _el=%errorlevel%
...
if defined _unattended (
echo.
call :_color %_Red% "网络未连接，但仍可继续执行。"
) else (
%eline%
...
call :_color %Red% "1688端口在您的连接中被阻止。"
echo.
...
echo:
echo   Online KMS Activation Script is a part of '微软激活脚本【由Seimo汉化】' [MAS] project.
echo:   
...
call :_color2 %_White% "     " %Red% "检查网络连接  [未连接]"
)
...
echo      Notes:
echo:
call :_color2 %_White% "     - " %Gray% "请确保您的网络已连接。"
call :_color2 %_White% "     - " %Gray% "请确保 Windows 更新正常运行。"
echo:
echo %line%
echo:
choice /C:09 /N /M ">    [9] 继续 [0] Go back : "
...
echo:
call :_color %Gray% "CBS 和 DISM 日志会复制到桌面上的 AT_Logs 文件夹。"
goto :at_back
...
title  修复 Licensing ^(ClipSVC ^+ Office vNext ^+ SPP ^+ OSPP^)
...
echo:
call :_color2 %_White% "      - " %Red% "只在必要时使用。"
echo:
echo %line%
echo:
...
echo:
call :_color %_Yellow% "将弹出一个窗口，您需要在该窗口中选择 [Quick] 选项..."
...
if exist "%SystemRoot%\Servicing\Packages\Microsoft-Windows-Server*Edition~*.mum" (
%eline%
echo WMI rebuild is not recommended on Windows Server. 程序中止...
goto :at_back
)

for %%# in (wmic.exe) do @if "%%~$PATH:#"=="" (
%eline%
echo wmic.exe file is not found in the system. 程序中止...
goto :at_back
)
...
if not defined error (
echo [工作中]
echo No need to apply this option. 程序中止...
goto :at_back
)
...
if defined _corrupt (
%eline%
echo Winmgmt service is corrupted. 程序中止...
goto :at_back
)
...
echo 未找到激活 ID，程序终止...
echo:
echo 查看此页获取帮助 %mas%troubleshoot
goto ced_done
)
)
...
DISM /English /Online /Get-CurrentEdition %nul%
cmd /c exit /b !errorlevel!
echo DISM 命令失败 [Error Code - 0x!=ExitCode!]
echo 检测到错误的操作系统版本，程序终止...
echo:
echo 查看此页面寻求帮助。 %mas%troubleshoot
goto ced_done
)
...
if defined _target (
for %%# in (%_target%) do (
echo %%# | findstr /i "特定国家的版本" %nul% || (set "_ntarget=!_ntarget! %%#")
)
)
...
%line%
echo:
call :dk_color %Gray% "您可以将版本 [%osedition%] [%winbuild%] 更改为以下其中之一。"
if defined dismnotworking (
call :dk_color %_Yellow% "注意 - DISM.exe 没有响应。"
if /i "%osedition:~0,4%"=="Core" call :dk_color %_Yellow% "     - 一旦更改为专业版，您将看到更多版本选项供您选择。"
)
%line%
echo:
...
echo [0]  %_exitmsg%
echo:
call :dk_color %_Green% "在键盘上输入选项编号，然后按 "Enter"："
set /p inpt=
...
echo:
echo  - 请保存您的工作后再继续，系统将自动重启。
echo:
echo  - 更改版本后，您需要使用 HWID 选项进行激活。
echo:
...
if %_dismapi%==1 (
echo:
echo 使用 %_chan% 密钥 %key% 应用 DISM API 方法
echo:
